...
if isfield(subj_data, 'tones') & subj_data.tones.done
  fprintf('\n\n\nYou already did the tones run. Are you sure you want to run it again?\n');
  x = input('If you want to redo the tones run, enter "YES". ', 's');
...
if ~isfield(subj_data, 'audiobook') ~ subj_data.audiobook.done
  error('Audiobook run must be run first!');
end %if
